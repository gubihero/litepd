(function(){
  ...
  app.controller('pokemonController', function($scope, $http, $routeParams){
    ...
    $http.get('https://pokeapi.co/api/v1/pokemon/'+pokemonName).success(function(data) {
      $scope.pokemon = data;
      console.log($scope.pokemon);
      $http.get('https://pokeapi.co'+$scope.pokemon['sprites'][0]['resource_uri']).success(function(data) {
        $scope.image = 'http://pokeapi.co'+data.image;
      });
      $http.get('https://pokeapi.co'+$scope.pokemon['descriptions'][0]['resource_uri']).success(function(data) {
        $scope.bio = data;
      });
    });
  });
